...
// 检查服务器是否安装过zookeeper
func ZookeeperInstallCheck(ip string) bool {
  res := true
  ssh := "root@" + ip
  // 查看是否有zookeeper进程
  cmd := exec.Command("ssh", ssh, "ps aux |grep zookeeper |grep -v grep | wc -l")
...
// 上传配置文件和包到服务器
func ZookeeperPackageUpload(ip string, version string) bool {
	res := true
	ssh := "root@" + ip
	...
	packagedir := dir + "/packages/zookeeper/"
	...
	// 上传配置文件
	command = "scp " + packagedir + "zoo.cfg " + ssh + ":" + destinationpath
	cmd = exec.Command("/bin/bash", "-c", command)
	_,err = cmd.Output()
	if err != nil {
			 logger.Error(err)
